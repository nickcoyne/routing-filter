...
[![CI](https://github.com/nickcoyne/routing-filter/actions/workflows/ci.yml/badge.svg)](https://github.com/nickcoyne/routing-filter/actions/workflows/ci.yml)
...
This version of `routing-filter` requires `Rails >= 8.1`.

For older Rails versions, use the `0.x` releases.
...
Thus routing-filter will make your :custom filter the _inner-most_ filter, wrapping the application _first_. The :common filter from your engine will be wrapped _around_ that onion and will be made the _outer-most_ filter.
...
- all the url_helpers that are generated by named routes as well as url_for continue to work in
...
Instead of trying to change things _between_ the URL recognition and
generation stages to achieve the desired result it _wraps around_ the whole
...
This way we can leave _everything_ else completely untouched.
...
- Finally we can accept URLs that have been generated by the url_helpers and,
